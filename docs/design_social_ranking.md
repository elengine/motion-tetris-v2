...
**ステータス**: 仕様確定版 v1.0（2026-09-07）— kokuten さんの論点回答を反映。開発着手承認待ち
...
## 7. ブラッシュアップ結果（2026-09-07 承認済み仕様）

1. **名前の初期値**: ✅ **Google の表示名をそのまま使用**（初回ログイン時に Google 名で profiles を自動作成。ゲーム内での名前変更で上書き可能）
2. **スプリントの順位基準**: ✅ タイム昇順（短いほど上位）
3. **匿名での履歴**: ✅ Round 1 はログイン必須（匿名履歴は Round 2 で検討）
4. **ランキング表示件数**: ✅ 上位20件
5. **認証プロバイダ**: ✅ Round 1 は Google のみ / LINE は Round 2
6. **BEST 表示**: ✅ **ログイン時はクラウドの自分の最高スコアに切替**（未ログインは従来の localStorage）
   - **自動セッション復元**: ブラウザ/タブを開き直した際に `supabase.auth.getSession()` で**自動認証**し、再ログイン操作なしでクラウド BEST/履歴/ランキングに復帰する
   - 端末差の注意: BEST の切り替えにより、ログイン前後で BEST 表示が変わる。これは仕様どおり。
...
```
アプリ起動（main() 冒頭）
  └─ supabase.auth.getSession()         ← localStorage のセッション復元・自動認証
       ├─ session あり → profiles 取得（display_name / クラウドBEST/履歴）
       │                   hudBest をクラウド値で更新、UI にログインアイコン表示
       └─ session なし → 未ログイン UI（従来動作、localStorage BEST）
  └─ onAuthStateChange の監視でログイン/ログアウトを即時反映
```

- PWAインストール後も同様に自動復元される
- トークン失効時は自動更新（supabase-js 標準）
